...
namespace _11._7
{
	...
	internal class Manager : Consultant, IManager
	{
		...
		public override void AddClient(List<Client> clients, string filePath, string worker, string newPhoneNumber = null,
			string newFamilyName = null, string newFirstName = null, string newLastName = null, string newSerialNumberDoc = null)
		{
			if (string.IsNullOrEmpty(newPhoneNumber) || string.IsNullOrEmpty(newFamilyName) || string.IsNullOrEmpty(newFirstName) || string.IsNullOrEmpty(newLastName)
				|| string.IsNullOrEmpty(newSerialNumberDoc))
				{
				MessageBox.Show("Все поля обязательны к заполнению при добавлении нового пользователя");
				return;
				}
		...
		public override void DeleteClient(List<Client> clients, Client selectedClient, string filePath)
		{
			if (selectedClient != null)
			{
